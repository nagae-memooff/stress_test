...
begin
  ...
  times = []
  # log response, 0
  user_ids = ( response[:items].map {|item| {id: item[:id], emp_code: item[:emp_code], email: item[:email], login_name: item[:login_name]} })[offset..-1]

  threads = []
  online_users_at_same_time.times do |n|
    thread = Thread.new do
      user1 = User.loop_login({login_name: user_ids[n][:email], password: 111111}, ( online_users_at_same_time * rand / 30 ).to_i)
